...
//jwg - can we go dynamic rather than preprocessor for output?
extern int (*outputChar)(int);
#define UNITY_OUTPUT_CHAR(a) outputChar(a)
...
struct _Unity
{
    ...
    unsigned char Verbose;//jwg
    const char* GroupFilter;//jwg
    const char* NameFilter;//jwg
    int RepeatCount; //jwg
    jmp_buf AbortFrame;
};
...
//jwg, some won't be needed when fixture stuff is integrated
#define UNITY_FAIL_AND_BAIL   { Unity.CurrentTestFailed  = 1; UNITY_OUTPUT_CHAR('\n'); longjmp(Unity.AbortFrame, 1); }
extern const char* UnityStrExpected;
extern const char* UnityStrWas;
void UnityTestResultsFailBegin(UNITY_LINE_TYPE lineNumber);
void UnityPrintNumberByStyle(const _US32 number, const UNITY_DISPLAY_STYLE_T style);
void UnityAddMsgIfSpecified(const char *);

...
{
    savedVerbose = Unity.Verbose;
    savedRepeat = Unity.RepeatCount;
    savedName = Unity.NameFilter;
    savedGroup = Unity.GroupFilter;
}

TEST_TEAR_DOWN(UnityCommandOptions)
{
    Unity.Verbose = savedVerbose;
    Unity.RepeatCount= savedRepeat;
    Unity.NameFilter = savedName;
    Unity.GroupFilter = savedGroup;
}
...
TEST(UnityCommandOptions, DefaultOptions)
{
    ...
    TEST_ASSERT_EQUAL(0, Unity.Verbose);
    TEST_ASSERT_POINTERS_EQUAL(0, Unity.GroupFilter);
    TEST_ASSERT_POINTERS_EQUAL(0, Unity.NameFilter);
    TEST_ASSERT_EQUAL(1, Unity.RepeatCount);
}
...
TEST(UnityCommandOptions, OptionVerbose)
{
    ...
    TEST_ASSERT_EQUAL(1, Unity.Verbose);
}
...
TEST(UnityCommandOptions, OptionSelectTestByGroup)
{
    ...
    STRCMP_EQUAL("groupname", Unity.GroupFilter);
}
...
TEST(UnityCommandOptions, OptionSelectTestByName)
{
    ...
    STRCMP_EQUAL("testname", Unity.NameFilter);
}
...
TEST(UnityCommandOptions, MultipleOptions)
{
    ...
    STRCMP_EQUAL("groupname", Unity.GroupFilter);
    STRCMP_EQUAL("testname", Unity.NameFilter);
    TEST_ASSERT_EQUAL(98, Unity.RepeatCount);
}
...
TEST(UnityCommandOptions, MultipleOptionsDashRNotLastAndNoValueSpecified)
{
    ...
    TEST_ASSERT_EQUAL(1, Unity.Verbose);
    STRCMP_EQUAL("gggg", Unity.GroupFilter);
    STRCMP_EQUAL("tttt", Unity.NameFilter);
    TEST_ASSERT_EQUAL(2, Unity.RepeatCount);
}
...
{
    void * m = malloc(10);
    UT_PTR_SET(outputChar, UnityOutputCharSpy_OutputChar);
    UnityMalloc_EndTest();
    UnityPointer_UndoAllSets();
